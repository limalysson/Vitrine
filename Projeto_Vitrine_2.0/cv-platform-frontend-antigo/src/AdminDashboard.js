...
function AdminDashboard({ onAdminLogout }) {
    ...
    const handleStatusChange = async (id, newStatus) => {
        ...
        try {
            // Usa a variável API_BASE_URL
            const response = await api.put(`${API_BASE_URL}/api/admin/curriculos/${id}/status`, { status: newStatus }, {
                headers: {
                    Authorization: `Bearer ${adminToken}`
                }
            });
    ...
    const handleSelectForCompany = async (id) => {
        ...
        try {
            // Usa a variável API_BASE_URL
            const response = await api.put(`${API_BASE_URL}/api/admin/curriculos/${id}/select`, null, {
                headers: {
                    Authorization: `Bearer ${adminToken}`
                }
            });
            ...
            if (response.data.selecionadoParaEmpresa) {
    ...
    // ========================================================================
    // NOVA ESTRUTURA: useEffect para limpar mensagens após 3 segundos
    // ========================================================================
    useEffect(() => {
        if (message) {
