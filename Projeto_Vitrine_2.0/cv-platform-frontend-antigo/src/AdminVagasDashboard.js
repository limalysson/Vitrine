...
function AdminVagasDashboard() {
    ...
    return (
      <main className="max-w-6xl mx-auto p-6">
        {message && (
          <div className="fixed top-6 right-6 z-50 bg-gray-800 text-white px-4 py-2 rounded shadow">{message}</div>
        )}
            ...
            {vagas.map(vaga => (
              <article key={vaga._id} className="bg-[rgba(26,35,126,0.18)] border border-white/20 rounded-xl p-5 shadow-md">
                <header className="flex items-start justify-between gap-4">
                  <div>
                    <h2 className="text-xl font-semibold text-white">{vaga.titulo} <span className="text-sm text-white/80">({vaga.status})</span></h2>
                  ...
                  <span className={`px-2 py-1 rounded text-xs font-medium ${vaga.status === 'ativo' ? 'bg-green-600 text-white' : 'bg-yellow-500 text-white'}`}>{vaga.status.toUpperCase()}</span>
                </header>
