    ...
    const fetchCurriculo = async () => {
      try {
        ...
        if (data?.success && data.curriculo) {
          ...
          // Garante que se vierem vazios do banco, a UI mostre pelo menos um bloco em branco (padrão)
          if (!curriculo.experiencias || curriculo.experiencias.length === 0) {
            curriculo.experiencias = [{ empresa: "", cargo: "", inicio: "", fim: "", descricao: "" }];
          }
          if (!curriculo.idiomas || curriculo.idiomas.length === 0) {
            curriculo.idiomas = [{ idioma: "", nivel: "" }];
          }
          if (!curriculo.projetos || curriculo.projetos.length === 0) {
            curriculo.projetos = [{ nome: "", descricao: "", link: "" }];
          }
      ...
      <div className="w-full text-white">
        <main className="w-full max-w-5xl mx-auto px-4 py-8 animate-fade-in-up">
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            {/* Seção de Foto */}
            <section className="card-default-large w-full relative overflow-hidden group border-white/5">
            ...
            {/* Dados Pessoais */}
            <section className="card-default-large w-full relative overflow-hidden group border-indigo-500/10 hover:border-indigo-500/30 transition-colors">
            ...
            {/* Experiências */}
            <section className="card-default-large w-full relative border-indigo-500/10">
              <h3 className="text-xl font-bold text-white mb-6 flex items-baseline gap-2 pb-4 border-b border-white/10">
              ...
              <div className="space-y-8">
                {formData.experiencias.map((exp, i) => (
                  <div key={i} className="bg-[rgba(15,23,42,0.4)] p-6 rounded-xl border border-white/5 relative group/item">
                    ...
                    {formData.experiencias.length > 1 && (
                      <button type="button" onClick={() => removeArrayItem(i, "experiencias")} className="absolute -right-3 -top-3 w-8 h-8 bg-slate-800 rounded-full border border-white/10 flex items-center justify-center text-rose-400 hover:text-white transition-all shadow-lg hover:bg-rose-500/80 group-hover/item:scale-110 z-20" title="Remover">
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-between items-center border-t border-white/5 pt-6">
                  <button type="button" onClick={() => addArrayItem("experiencias")} className="w-full sm:w-auto px-5 py-2.5 rounded-lg bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 font-semibold hover:bg-indigo-500/20 transition-all text-sm flex justify-center items-center gap-2">
            ...
            <section className="card-default-large w-full relative border-indigo-500/10">
                ...
                {formData.idiomas.map((idioma, i) => (
                  <div key={i} className="bg-[rgba(15,23,42,0.4)] p-4 rounded-xl border border-white/5 relative group">
                    ...
                    {formData.idiomas.length > 1 && (
                      <button type="button" onClick={() => removeArrayItem(i, 'idiomas')} className="absolute -right-3 -top-3 w-8 h-8 bg-slate-800 rounded-full border border-white/10 flex items-center justify-center text-rose-400 opacity-0 group-hover:opacity-100 transition-all shadow-lg hover:text-white hover:bg-rose-500/80 z-20 cursor-pointer" title="Remover">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4"><path strokeLinecap="round" strokeLinejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" /></svg>
                      </button>
                    )}
                  </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-between items-center border-t border-white/5 pt-6">
                <button type="button" onClick={() => addArrayItem("idiomas")} className="w-full sm:w-auto px-5 py-2.5 rounded-lg bg-teal-500/10 border border-teal-500/20 text-teal-400 font-semibold hover:bg-teal-500/20 transition-all text-sm flex justify-center items-center gap-2">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4"><path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" /></svg>
                  Adicionar Idioma
                </button>
            ...
            <section className="card-default-large w-full relative border-indigo-500/10">
                ...
                {formData.projetos.map((p, i) => (
                  <div key={i} className="bg-[rgba(15,23,42,0.4)] p-5 rounded-xl border border-white/5 relative group/item">
                    ...
                    {formData.projetos.length > 1 && (
                      <button type="button" onClick={() => removeArrayItem(i, 'projetos')} className="absolute -right-3 -top-3 w-8 h-8 bg-slate-800 rounded-full border border-white/10 flex items-center justify-center text-rose-400 hover:text-white hover:bg-rose-500/80 transition-all opacity-0 group-hover/item:opacity-100 shadow-lg z-20 cursor-pointer" title="Remover">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4"><path strokeLinecap="round" strokeLinejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" /></svg>
                      </button>
                    )}
                  </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-between items-center border-t border-white/5 pt-6">
                <button type="button" onClick={() => addArrayItem("projetos")} className="w-full sm:w-auto px-5 py-2.5 rounded-lg bg-amber-500/10 border border-amber-500/20 text-amber-400 font-semibold hover:bg-amber-500/20 transition-all text-sm flex justify-center items-center gap-2">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4"><path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" /></svg>
            ...
            {/* Anexo PDF */}
            <section className="card-default-large w-full relative border-indigo-500/10 mb-12">
              <h3 className="text-xl font-bold text-white mb-6 flex items-baseline gap-2 pb-4 border-b border-white/10">
            ...
            {/* Action Final Geral */}
            <div className="sticky bottom-6 z-40 bg-slate-900/90 backdrop-blur-xl border border-indigo-500/20 py-3 px-6 rounded-xl flex flex-col sm:flex-row items-center justify-between gap-4 shadow-2xl max-w-5xl mx-auto">
              <p className="text-slate-400 text-xs hidden md:block leading-relaxed max-w-2xl">
                Recomendamos que os botões "Salvar Bloco" sejam utilizados frequentemente para evitar perder dados, mas por este painel você pode confirmar todas as áreas preenchidas de uma só vez.
              </p>
              <button type="submit" disabled={isLoading} className="w-full sm:w-auto home-button-card !py-2 !px-6 text-sm shadow-indigo-500/25 !from-indigo-600 !to-purple-600 hover:!from-indigo-500 hover:!to-purple-500 flex justify-center items-center gap-2">
                {isLoading ? (
